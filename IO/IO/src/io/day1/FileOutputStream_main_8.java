...
public class FileOutputStream_main_8 {
...
*/   
	...
	public static void main(String[] args) {
		...
		System.out.println(">> 내용을 입력하세요[입력하신 내용은 C:\\iotestdata\\result_2.txt 파일에 저장됨] ");	// \ 한개만 쓰면 escape임
		
		String fileName = "C:/iotestdata/result2.txt"; // OS가 windows 이외에 / 이기 때문에, 다 호환되게 하기 위해 /를 사용한다.
		...
		try {
			int inputLength = 0;	// 읽어들인 크기가 얼마입니까?
		...
		//	boolean append = true;			
			...
			*/
			...
			while( (inputLength = System.in.read(dataArr)) != -1 ) {
				fost.write(dataArr, 0, inputLength);		// print를 쓰면 int로 나오므로, write를 쓴다.
				fost.flush();
				
				totalByte += inputLength;	// 누적 몇 byte 인가?
				cnt++;	// 반복횟수			
			}// end of while------------
